	...
	Category
	{
		...
		SubShader
		{
			Pass
			{
				...
				inline float CalcVolumeFogIntensity(float3 sphereCenter, float sphereRadius, float innerRatio, float density, float3 cameraPosition, float3 viewDirection, float maxDistance)
				{
				...
				half4 frag(v2f i) : COLOR
				{
					...
					float fog = CalcVolumeFogIntensity(FogParam.xyz * 0.001, FogParam.w * 0.001, _InnerRatio, _Density * 1000, _WorldSpaceCameraPos * 0.001, viewDir, depth * 0.001);
